...
`endpointing_sensitivity` controls how the bot determines the when the human is done speaking. It can be one of **sensitive**, **relaxed**, or **auto** (default and currently equivalent to relaxed). 
...
***Note:** the amount of time the agent waits during a pause is adjusted based on the human's utterance speed. If the human speaks fast, the bot will wait less, and if the human speaks slow the bot will wait longer.*
...
`conversation_speed` controls when the bot will begin responding to the human after they finish speaking. This includes normal utterances from the human as well as interruptions.
